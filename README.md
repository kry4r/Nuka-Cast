...
- **TVBox 影视聚合**：兼容单仓和多仓、普通 JSON、前导注释/Base64 配置、CMS、CatVod JAR Spider 与 ES Module JS Spider；多仓自动导入子仓，搜索默认选择最快的健康仓。
...
首次安装默认加入小盒子多仓 `http://xhztv.top/dc`；多仓目录加载后会自动同步所有子仓。搜索时只查询当前选中的仓，默认优先最近可用且响应最快的仓，也可手动切换。更多配置源请打开电视“投屏”页显示的网页控制地址，输入六位配对码后进入“源管理”，填写名称和 `http(s)` 配置地址进行添加、删除或刷新；上游接口可用性会随时间变化。Android 4.4 会显式启用 TLS 1.2，所有应用网络与 AirPlay 媒体套接字只使用 IPv4。
