...
首次安装默认加入饭太硬 TVBox 配置；旧版本若留下空列表，升级后会自动修复一次。也可在电视端“设置”选择“恢复内置源”。更多配置源请打开电视“投屏”页显示的网页控制地址，输入六位配对码后进入“源管理”，填写名称和 `http(s)` 配置地址进行添加、删除或刷新；上游接口可用性会随时间变化。
...
- SMB 当前由 jCIFS 1.3.17 提供 SMB1 兼容；仅启用 SMB2/SMB3 的 NAS 需要后续升级客户端实现。
