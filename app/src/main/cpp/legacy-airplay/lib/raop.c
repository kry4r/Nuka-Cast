...
int
raop_get_public_key_hex(raop_t *raop, char *output, int output_len)
{
    static const char hex[] = "0123456789abcdef";
    unsigned char public_key[32];
    int i;
    if (!raop || !output || output_len < 65) return -1;
    pairing_get_public_key(raop->pairing, public_key);
    for (i = 0; i < 32; i++) {
        output[i * 2] = hex[(public_key[i] >> 4) & 0x0f];
        output[i * 2 + 1] = hex[public_key[i] & 0x0f];
    }
    output[64] = '\0';
    return 0;
}
