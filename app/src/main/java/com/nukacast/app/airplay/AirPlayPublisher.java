...
final class AirPlayPublisher {
    ...
    synchronized void start(int port, String publicKey) throws Exception {
        ...
        jmdns.registerService(ServiceInfo.create("_airplay._tcp.local.", "NukaCast", port, 0, 0, airplay));
        jmdns.registerService(ServiceInfo.create("_raop._tcp.local.", compactMac + "@NukaCast", port, 0, 0, raop));
    }
